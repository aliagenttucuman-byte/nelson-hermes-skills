...
metadata:
  hermes:
    ...
    related_skills: [nelson-project-constitution, spec-driven-development, writing-plans, ponytail, subagent-driven-development, requesting-code-review, nelson-code-quality, nelson-project-bootstrap, nelson-pricing-model]
---
...
### 4. PLANEAR → `writing-plans` + `ponytail`
...
**Ponytail integrado:** Antes de definir dependencias o componentes, pasar cada decisión por la escalera:
1. ¿Necesita existir? → ¿Es YAGNI?
2. ¿Stdlib/plataforma lo cubre? → No agregar dep
3. ¿Dep ya instalada? → No agregar nueva
4. ¿Una función alcanza? → No crear clase/módulo

Si Tony dice "se ensució" durante el planning → activar `ponytail ultra`.

**Salida:** `PLAN.md` con:
- Arquitectura de componentes
- Dependencias y versiones (solo las que pasaron la escalera ponytail)
...
### 5c. PONYTAIL REVIEW → `ponytail` (anti-bloat gate)

**Cuándo:** Después de Fase 4 (PLANEAR), antes de Fase 6 (ANALIZAR).
**Quién:** JARVIS automáticamente

**Qué hace:** Revisa el PLAN.md por over-engineering antes de que se convierta en tareas y código.

Preguntas obligatorias:
- ¿Hay abstracciones que no fueron pedidas explícitamente?
- ¿Hay dependencias nuevas que stdlib/plataforma/dep instalada ya cubre?
- ¿Hay capas o módulos con un solo caller?
- ¿Hay scaffolding "para después" que es YAGNI hoy?

Si el plan pasa limpio → seguir. Si hay hallazgos → reportar con formato ponytail-review y ajustar plan.
