...
def get_or_create_session(session_id: str) -> str:
    """Crea la sesión si no existe, devuelve el id."""
    if session_id == "auto":
        session_id = datetime.now().strftime("%Y-%m-%d-%H%M")

    # Intentar crear (idempotente si ya existe)
...
def store(session_id: str, user_msg: str, jarvis_msg: str, tags: list[str] = None):
    ...
    res = api("POST",
              f"/v3/workspaces/{WORKSPACE}/sessions/{sid}/messages",
              {"messages": messages})
...
if __name__ == "__main__":
    ...
    parser.add_argument("--session", default="auto",
                        help="ID de sesión (o 'auto' para timestamp)")
    parser.add_argument("--user", default="", help="Mensaje del usuario")
    parser.add_argument("--jarvis", default="", help="Respuesta de JARVIS")
    parser.add_argument("--tags", default="",
                        help="Tags separados por coma: decision,arquitectura")
