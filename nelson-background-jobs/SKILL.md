...
### Tarea registrada pero no ejecutada → "unregistered task" en logs
Si el worker recibe un mensaje pero lo descarta con `KeyError` en `strategies`, la causa es que el módulo de la tarea **no está en `include`** de `celery_app`. El log exacto:
```
Received unregistered task of type 'mytask.run'
The message has been ignored and discarded.
```
Fix: agregar el módulo a `include` y reiniciar el worker:
```python
celery_app = Celery(
    "myapp",
    broker=...,
    include=["app.tasks.existing_task", "app.tasks.new_task"],  # ← nuevo módulo aquí
)
```
La tarea queda en cola Redis mientras el worker no la conoce — **no se pierde**, solo se discarda. Re-encolar después de fix.

### NaN/Inf en result_json rompe columna JSON de Postgres
Si la tarea serializa un `float('nan')` o `float('inf')` en un campo `JSON` de Postgres:
```
sqlalchemy.exc.DataError: invalid input syntax for type json
Token "NaN" is invalid.
```
Sanitizar siempre antes de guardar:
```python
import math
val = result.get("area_ha", 0) or 0
