...
## 12. Lean Cortex (lean-ctx) — Compresión de Contexto

lean-ctx es un binario Rust que actúa como servidor MCP y capa de shell hooks. Reduce el consumo de tokens al comprimir lecturas de archivos y output de comandos — complementario a OptiLLM (que optimiza la inferencia backend).

| Herramienta | Nivel | Función |
|---|---|---|
| **lean-ctx** | Capa de tools del agente | Comprime lecturas de archivos y shell output |
| **OptiLLM** | Proxy de inferencia | Aplica MCTS/MOA/Re2 antes de enviar al modelo |

Ambas actúan en capas diferentes; se potencian mutuamente.

### Estado en el servidor (ai-server)

- Binario: `/home/server/.local/bin/lean-ctx` (versión 3.6.16)
- MCP configurado en: `~/.hermes/config.yaml`
- Shell hooks en: `~/.bashrc`
- Daemon como servicio systemd de usuario (arranca automáticamente)
...
| Tool MCP | En vez de | Ahorro aprox |
|---|---|---|
| `ctx_read(path, mode)` | `read_file` / `cat` | Re-reads ~13 tok (vs ~2000) |
| `ctx_shell(command)` | `terminal` / bash | git status ~120 tok (vs ~800) |
| `ctx_search(pattern, path)` | `search_files` / grep | Resultados compactos |
| `ctx_tree(path, depth)` | `ls` / find | Mapa compacto |

Los tools MCP se cargan automáticamente al iniciar sesión en Hermes.
...
### Proyectos ya inicializados

```
~/jarvis-demo-shell/
~/brainstorming/2026-05-21-deepagents-spike/
~/brainstorming/2026-05-19-forestai-poc/
~/brainstorming/2026-05-22-fleet-optimizer/
```

### Pitfalls lean-ctx

- Requiere git repo para crear AGENTS.md — hacer `git init` primero si el dir no es repo
- Los tools MCP se cargan al inicio de sesión; cambios en config requieren nueva sesión
- El ahorro en `lean-ctx gain` empieza en 0% — el caché está vacío en las primeras sesiones
- El daemon offline (`lean-ctx serve -d`) permite tracking persistente; activarlo como servicio systemd es lo recomendado
