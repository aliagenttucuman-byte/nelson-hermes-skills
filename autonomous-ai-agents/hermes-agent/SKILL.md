...
> **Pitfall — activar un provider específico:** `hermes memory setup <nombre>` NO acepta argumentos de provider. El wizard interactivo setea "built-in only". Para activar un provider concreto usar:
> ```bash
> hermes config set memory.provider holographic   # o hindsight, mem0, honcho, etc.
> hermes memory status                             # verificar
> ```
> Verificado en producción (2026-05-21). Requiere reiniciar la sesión para que cargue el plugin.
...
For endpoints that are not built-in, add entries under `custom_providers:` in `config.yaml`. Each entry is a YAML list item (prefixed with `-`).

| Field | Required | Description |
|-------|----------|-------------|
| `name` | Yes | Display name. Use this with `hermes chat -m "custom:<name>"` or `/model custom:<name>`. |
| `base_url` | Yes | Full API base URL (with scheme). Aliases: `api`, `url`. |
| `model` or `default_model` | No | Default model to use when none is specified. |
| `api_key` | No | Inline API key (not recommended — use `key_env` instead). |
| `key_env` | No | Name of an environment variable in `~/.hermes/.env` that holds the key. Hermes reads it at runtime. Aliases: `api_key_env`. |
| `api_mode` or `transport` | No | Protocol/transport to use. Determines which SDK Hermes instantiates. |

**Transport values (`api_mode` / `transport`):**

| Value | SDK used | When to use |
|-------|----------|-------------|
| `openai_chat` | OpenAI SDK (default) | OpenAI-compatible endpoints: OpenRouter, Ollama, local servers, most proxies. |
| `anthropic_messages` | Anthropic SDK | Claude endpoints, including Azure AI Foundry Claude. Required for Azure Anthropic because Hermes's Anthropic adapter (`agent/anthropic_adapter.py`) auto-detects `azure.com` URLs and injects `api-version=2025-04-15` + `x-api-key` header. |
| `codex_responses` | Codex SDK | OpenAI Codex endpoints only. |

**Best practices:**
- Always use `key_env` and store the actual key in `~/.hermes/.env`, never inline in `config.yaml`.
- The `api_mode` field is snapshotted at session startup. Switching providers via `/model` within the same session does NOT change `api_mode`. Start a new session (`/reset` or `hermes chat -m ...`) for `api_mode` changes to take effect.
- Hermes auto-detects `api_mode` from the URL when omitted (e.g. `/coding` → `anthropic_messages`, `openrouter.ai` → `openai_chat`), but for Azure AI Foundry Claude it is safer to set it explicitly.

**Example — Azure AI Foundry Claude (verified working):**
...
# or mid-session: /model custom:azure-claude  then /reset
```

**Pitfall — Azure Foundry GPT-5.x / Codex (Responses API):**

Azure AI Foundry endpoints at `*.cognitiveservices.azure.com/openai/responses` require two things that the OpenAI SDK does not provide when used as a generic client:

1. **`api-key` header** instead of `Authorization: Bearer <key>`. The standard `openai.OpenAI` client always sends `Authorization: Bearer`, which Azure rejects. Unlike `openai.azure.com`, Hermes has no built-in special-case adapter for `cognitiveservices.azure.com`.
2. **`?api-version=2025-04-01-preview`** query parameter. The OpenAI SDK constructs `{base_url}/responses` without it, and Azure returns HTTP 404 (not 401 — misleading).

**Status — RESOLVED with local patch.** A ~45-line patch to `run_agent.py` adds `_is_azure_cognitive_url()` detection and routes these endpoints through `openai.AzureOpenAI` with the correct `api_version`. Both Azure Claude and Azure GPT/Codex are confirmed working (2026-05-26). The patch is local to `~/.hermes/hermes-agent/run_agent.py` and must be re-applied after Hermes updates.

| Path | Status | Description |
|------|--------|-------------|
| **A. Patch Hermes** | ✅ Working | Detect `cognitiveservices.azure.com` in `_create_openai_client()` and use `openai.AzureOpenAI(api_version="2025-04-01-preview")`. See `references/azure-foundry-codex.md` for the exact diff and `templates/azure-cognitive-patch.py` for a re-usable patch script. |
| **B. Local proxy** | Alternative | FastAPI proxy translating `Authorization: Bearer` → `api-key:` and injecting `?api-version=`. Still valid if you prefer not to patch Hermes source. |
| **C. Azure deployment setting** | Context-dependent | Some Azure Foundry deployments also expose GPT-5.x on `/chat/completions`. Check your deployment; if available, `api_mode: openai_chat` + `default_headers: {"api-key": "..."}` may work without patches. |

**Quick usage (after patch):**
...
**Important:** The patch strips a trailing `/openai` from `base_url` before passing it as `azure_endpoint`, because the Azure OpenAI SDK appends `/openai/` internally. Keep `/openai/` in your `config.yaml` — the patch handles the cleanup.

**Pitfall — `/openai/models` ≠ deployed models.** The endpoint `/openai/models?api-version=...` returns ALL models in the Azure global catalog, not what's actually deployed on your resource. A model appearing there does NOT mean it's ready to use. Always verify with `/openai/deployments` — if that returns `"data": []`, the model was never deployed and you'll get `DeploymentNotFound` regardless of patch or `api_version`.

See `references/azure-foundry-codex.md` for full technical details, the exact diff, curl verification commands, debugging checklist, and the re-usable patch template.
...
### HTTP 400 `reasoning_details` error (OpenCode Go + Kimi K2.6)

If sessions fail with `HTTP 400: Extra inputs are not permitted, field: messages[N].reasoning_details`, the OpenCode Go adapter is forwarding internal reasoning fields that the endpoint rejects. This happens specifically with models that produce extended thinking (Kimi K2.6 and similar).
...
```
Then start a new session. The `reasoning_details` fields are stripped by the Anthropic adapter and do not reach the API.

If you want to keep using OpenCode Go, switch to a non-reasoning model (e.g. `kimi-k2` without extended thinking).
...
5. **OpenCode Go + Kimi K2.6 → HTTP 400 `reasoning_details`**: The OpenCode Go adapter injects `reasoning_details` into message objects; downstream providers reject it with `Extra inputs are not permitted`. Fix: switch to a provider that doesn't inject reasoning metadata:
   ```bash
   hermes config set model.provider anthropic
   hermes config set model.default claude-sonnet-4-20250514
   hermes config set model.base_url ""
   hermes config set model.api_mode ""
   ```
   Then start a new session (`/reset` or relaunch). The `api_mode` is snapshotted at startup — mid-session changes don't take effect.
5. **OpenCode Go + Kimi K2.6 — HTTP 400 `reasoning_details` error**: Kimi K2.6 vía OpenCode Go incluye `reasoning_details` en los mensajes del historial. El provider lo rechaza con `Extra inputs are not permitted, field: messages[N].reasoning_details`. Fix: cambiar a Anthropic directo o a un modelo sin reasoning extendido. Comandos:
   ```bash
   hermes config set model.provider anthropic
   hermes config set model.default claude-sonnet-4-20250514
   hermes config set model.base_url ""
   hermes config set model.api_mode ""
   ```
   Requiere reiniciar la sesión (`/reset`) para que aplique. Este problema ocurre en sesiones largas donde el historial acumula mensajes con `reasoning_details` de Kimi.
...
## lean-ctx MCP Integration (active on this server)

lean-ctx v3.6.16 is installed and configured as an MCP server in `~/.hermes/config.yaml`:

```yaml
mcp_servers:
  lean-ctx:
    command: "/home/server/.local/bin/lean-ctx"
    env:
      LEAN_CTX_DATA_DIR: "/home/server/.config/lean-ctx"
```

The `~/.hermes/HERMES.md` and `~/.hermes/.hermes.md` files are auto-generated by lean-ctx and contain ctx_* usage rules. **Do not manually edit them** — they are regenerated by `lean-ctx init --agent hermes`.

When lean-ctx MCP tools are available in session, **prefer** `ctx_read`, `ctx_shell`, `ctx_search`, `ctx_tree` over native equivalents. See the `nelson-lean-ctx` skill for full reference.

To reinstall/reconfigure: `lean-ctx doctor --fix && lean-ctx init --agent hermes`

---

## User Workflow Preferences (Nelson / Tony Stark)

The following preferences were explicitly stated by Nelson and must be respected in all sessions:

### Skills/Memory Sync to GitHub

Nelson maintains a GitHub repo (`nelson-hermes-skills`) with scripts (`sync-to-repo.sh`, `sync-from-repo.sh`) to backup skills and memory.

**Preference:** Sincronizar skills y memoria al repo **solo cuando haya información valiosa o cambios importantes**. **NO** en cada modificación menor. El usuario decide cuándo es el momento de hacer backup.

**Behavior:**
- Do NOT automatically propose syncing after every small skill patch or memory update
- Do NOT run `sync-to-repo.sh` + `git push` as a reflex after every change
- Wait for Nelson to explicitly ask for a sync, or when the change is clearly substantial (new skill creation, major workflow discovery, critical data)
- Nelson prefers to control the backup timing himself

This preference is also stored in persistent memory for cross-session consistency.
