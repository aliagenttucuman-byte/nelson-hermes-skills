...
## Implementación Productiva — ai-server

El router declarativo está deployado en el servidor:
- Path: `/home/server/nelson/routing/`
- Puerto: 8743
- Systemd: `nelson-agent-router.service` (habilitado, arranca con el server)
- Health: `curl http://localhost:8743/health`
- API: `POST http://localhost:8743/route` con `{"goal": "..."}`
- Taxonomía: `GET http://localhost:8743/taxonomy`
- Repo: https://github.com/aliagenttucuman-byte/nelson-agent-router

El executor del meta-orquestador (:8744) consume este router y mapea cada
`RoutingResult.category` → agente responsable + skills a cargar.
Ver `nelson-meta-orchestrator/references/executor-real-diseno.md` para el mapeo completo.

## Pitfalls Descubiertos en Producción

### Sigmoide con parámetros incorrectos → score uniformemente bajo
El parámetro de la función sigmoide `confidence = 1 / (1 + math.exp(-k * (score - offset)))`
es muy sensible. Con `k=2, offset=2` y una sola keyword, el score era ~0.27 (bajo el threshold
de fallback de 0.45), provocando que TODAS las rutas tomaran el path de LLM fallback.
Valores calibrados en producción: `k=0.8, offset=1.5`.
Verificar siempre con casos simples: 1 keyword corta → ~0.7, 3 keywords → ~0.95.

### MULTI_AGENT_THRESHOLD muy alto (0.6) → nunca detecta multi-agente
Si el threshold está por encima del score típico de una keyword corta, la detección
multi-agente nunca se activa. Bajar a 0.55 para el umbral de producción.

### Puerto ocupado al arrancar systemd
Si previamente se lanzó el servicio en background (terminal/uvicorn directo),
el puerto 8743 queda tomado y systemd falla con "address already in use".
Antes de `systemctl start`:
```bash
kill $(lsof -ti:8743) 2>/dev/null || true
sudo systemctl start nelson-agent-router
```
