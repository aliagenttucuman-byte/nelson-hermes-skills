...
## Pitfall — `terminal("cat file")` corrompe secretos al reescribir archivos

**NUNCA** leer un archivo con `terminal("cat ...")` y luego usar ese output para
reescribir el archivo si contiene API keys, passwords, o cualquier secreto.

El output de `terminal()` **censura los valores** (ej: `NVIDIA_API_KEY=***`).
Si luego escribís ese output como nuevo contenido del archivo, la línea queda
truncada y el archivo falla con `SyntaxError: unterminated string literal`.

**Síntoma característico:** línea corrupta como `NVIDIA_API_KEY=os.get...EY")`

**Solución:** modificar archivos con Python directo via `terminal`:
```bash
python3 << 'PYEOF'
with open('archivo.py', 'r') as f:
    lines = f.readlines()
# Modificar lines[i] directamente (índice = número_línea - 1)
lines[20] = 'NVIDIA_API_KEY = os.getenv("NVIDIA_API_KEY")\n'
with open('archivo.py', 'w') as f:
    f.writelines(lines)
import ast; ast.parse(''.join(lines)); print("SYNTAX OK")
PYEOF
```

El heredoc `<< 'PYEOF'` (comillas en el delimitador) evita que el shell
expanda variables y preserva el contenido exacto.
